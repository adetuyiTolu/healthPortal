<%
if(session.getAttribute("reg").toString().isEmpty()){
 session.setAttribute("reg",request.getParameter("reg"));  
 session.setAttribute("status",request.getParameter("id")); 
}
if(session.getAttribute("status").toString().equalsIgnoreCase("in-patient")){
%>
<script>(window.location.href='inpatient.jsp');</script>
<%}%>
...
<div id="labf"><jsp:include page="searchservice.jsp"/></div>
  ...
  <div id="footer">
    <div id="bottom_addr">© 2013 Good Health Inc. All Rights Reserved</div>
  </div>
  </div>
</div>
<br />
